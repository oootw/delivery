...
/**
 * ElectronicCertificatePaymentData Class Doc Comment
 *
 * @category Class
 ...
 */
class ElectronicCertificatePaymentData implements ModelInterface, ArrayAccess, \JsonSerializable
{
     ...
     * @var string
     */
    protected static $openAPIModelName = 'ElectronicCertificatePaymentData';
     ...
     * Sets amount
     *
     * @param \YOOMONEY\Model\MonetaryAmount $amount Сумма, которую необходимо использовать по электронному сертификату, — значение totalCertAmount, которое вы получили в ФЭС НСПК в запросе на предварительное одобрение использования сертификата (Pre-Auth): https://www.nspk.ru/developer/api-fes#operation/preAuthPurchase. Сумма должна быть не больше общей суммы платежа (amount).
     *
     * @return self
     */
     ...
     * Sets basket_id
     *
     * @param string $basket_id Идентификатор корзины покупки, сформированной в НСПК, — значение purchaseBasketId, которое вы получили в ФЭС НСПК в запросе на предварительное одобрение использования сертификата (Pre-Auth): https://www.nspk.ru/developer/api-fes#operation/preAuthPurchase.
     *
     * @return self
     */
    public function setBasketId($basket_id)
    {
        ...
        if ((!preg_match("/[0-9]{24}/", ObjectSerializer::toString($basket_id)))) {
            throw new \InvalidArgumentException("invalid value for \$basket_id when calling ElectronicCertificatePaymentData., must conform to the pattern /[0-9]{24}/.");
        }
