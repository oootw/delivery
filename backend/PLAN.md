...
> **Статус M9: реализовано полностью (2026-07-09…11).** Домены `Promotion` (скидки+промокоды: типы Automatic/Promocode, target Order/Item/Category, полный набор условий, движок стекинга по priority+stackable) и `Loyalty` (кошелёк баллов/кэшбэк, уровни tiers по lifetime spend, штампы). Расчёт заказа развязан портами `Order/Pricing/OrderPricingInterface` и `Order/Rewards/OrderRewardsInterface`; общий `Order/Pricing/OrderPriceCalculator` — единая точка расчёта для оформления и предпросмотра (quote `POST /venues/{id}/orders/quote`). Гостевая витрина акций `GET /venues/{id}/promotions`, read-only разделы EasyAdmin (Promotion/LoyaltyProgram/LoyaltyTier/LoyaltyTransaction/StampProgram), метрики промо/баллов на дашборде, аудит новых сущностей. Миграции `Version20260709120000`…`Version20260710130000`. Полный журнал решений и под-этапов — [`PLAN_PROMOTIONS.md`](./PLAN_PROMOTIONS.md).

> **Статус M7: частично (2026-07-11).** ✅ **Транзакционность** — введён порт `Shared/Transaction/TransactionInterface` (адаптер `Infrastructure/Doctrine/Transaction/DoctrineTransaction` поверх `EntityManagerInterface::wrapInTransaction`); применён в `PlaceOrder` (заказ+скидки+резерв баллов атомарно), `CreateWorkspace` (воркспейс+owner-membership — устранён осиротевший воркспейс из долга M2) и `MenuImporter` (весь импорт одной транзакцией: атомарность + N коммитов→1 = батчинг). ✅ **Чистка** — удалён неиспользуемый `libs/YOOMONEY` (7.1 МБ) + `docs/yookassa` + PSR-4 из `composer.json` (**пользователю: `composer dump-autoload`**). ✅ **Unit-тесты PHPUnit** (`tests/Unit/`, 43 теста): PromotionEngine (стекинг), PromotionConditions (условия/happy-hours), TierResolver, LoyaltyProgram (earn/redeem = RewardCalculator), WaitTimeEstimator, PointsExpiryCalculator — `php vendor/bin/phpunit tests/Unit`. ✅ **Конкурентность резерва баллов** (пессимистичная блокировка `getOrCreateForUpdate`, аудит #11). ✅ **Сгорание баллов** (крон `app:loyalty:expire-points`, FIFO, аудит #8). ⏳ **Осталось:** функциональные тесты PlaceOrder (нужна тестовая БД), метрика «прогноз vs факт» для WaitTime. Полный статус двух волн фиксов — в `AUDIT.md`.

> **Логотипы точек (2026-07-11).** Конвенция хранения изображений: файлы в `public/upload/{slug}/` (slug воркспейса), отдаются веб-сервером по `/upload/*`, API возвращает URL (не стримит байты). Логотип точки — по имени `venue-{venueId}.{jpeg|jpg|png}` (без поля в БД, поиск glob'ом) через порт `Venue/Logo/VenueLogoStorageInterface` (адаптер `Infrastructure/Storage/VenueLogoStorage`). Actions: `GET /venues/{id}/logo` (любой авторизованный), `POST /venues/{id}/logo` (владелец, multipart `logo`, image/jpeg|png, ≤5 МБ). Будущие картинки (меню, лого воркспейса) — по той же конвенции.

> **Безопасность авторизации (2026-07-12).** Закрыты пункты аудита #15 и #10 (см. `AUDIT.md`). **#15 — реальная проверка SMS-кода** (была застаблена, любой код проходил): застабленные `CodeRepository::validateCode/validateCodeByCreatedAt` заменены на доступ к данным (`findActiveCode`/`hasRecentCode`/`save`), матчинг вынесен в домен (`Code::matches` через `hash_equals`, `isExpiredAt`, `isUsed`, попытки) и `CheckOntimeCodeHandler`. Согласовано: одноразовость, TTL 5 мин, **3 попытки** до сгорания, код 4 цифры, кулдаун **60 сек**. Колонки `used_at`/`attempts` в `code`, миграция `Version20260712120000`. **#10 — enumeration аккаунтов**: decoy-записи (код пишется для любого номера, SMS только зарегистрированному → лимит/кулдаун единообразны), **async-SMS** (порт `Authorize/Gateway/SmsSenderInterface` + `Infrastructure/Messenger/Sms` → тайминг веток выровнен, воркер `messenger:consume async`), единый ответ verify (`AuthorizeAction` не раскрывает «Пользователь не найден»). Unit-тесты `tests/Unit/Authorize` (12).

> **Платежи: мультипровайдер + per-workspace (2026-07-12).** Владелец может выбрать провайдера оплаты **заказов** в своём воркспейсе (CloudPayments или ЮKassa, на свой мерчант-аккаунт); подписки владельца — всегда платформенный CloudPayments. Контракты вынесены в `Shared/Contract/Payment` (см. `architecture.md` §9 и авто-память `payment-provider-abstraction`). Срезы: (1) `Billing/Entity/WorkspacePaymentSettings` (provider + шифрованные креды, one-per-workspace), owner-Actions `PUT/GET /workspaces/{id}/payment-settings`, миграция `Version20260712130000`; (2) порт `OrderPaymentGatewayInterface` + резолвер + реальный `Infrastructure/YooKassa/YooKassaOrderGateway` (embedded `confirmation_token`) + интеграция в `PlaceOrder` (ответ провайдер-зависимый), env `YOOKASSA_API_URL`; (3) `POST /webhooks/yookassa` — подтверждение перезапросом платежа (у ЮKassa нет подписи), матч заказа по `metadata.order_id` (`RecordOrderPayment` получил `?orderId`). **Отложено:** возврат/отмена платежа (симметрично — у CloudPayments тоже нет, общий долг с M5); функц. e2e платежей (тест-БД/песочница ЮKassa — долг M7).
